...
export default function WhyChooseUs() {
  ...
  return (
    <section ref={sectionRef} className="py-12 sm:py-16 md:py-24 bg-gradient-to-br from-raizing-cream-50 dark:from-gray-900 to-raizing-teal-900/30 dark:to-gray-800/30 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-8 sm:mb-12 md:mb-16 animate-on-scroll opacity-0">
          ...
          <p className="text-base sm:text-lg md:text-xl text-raizing-teal-900 dark:text-gray-300 max-w-3xl mx-auto px-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {features.map((feature, index) => (
            ...
            >
              <div className="relative h-full bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl p-6 sm:p-8 border border-raizing-teal-900/20 hover:border-raizing-maroon-500/50 transition-all duration-500 hover:transform hover:-translate-y-2 hover:shadow-xl">
                ...
                <div className="relative z-10">
                  <div className={`inline-flex p-3 sm:p-4 bg-gradient-to-r ${feature.gradient} rounded-lg sm:rounded-xl text-white mb-4 sm:mb-6 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-lg`}>
                    {feature.icon}
                  </div>

                  <h3 className="text-lg sm:text-xl font-bold text-raizing-teal-900 dark:text-white mb-2 sm:mb-3 group-hover:text-raizing-maroon-600 dark:group-hover:text-raizing-maroon-400 transition-colors">
                    {feature.title}
                  </h3>
                  <p className="text-sm sm:text-base text-raizing-teal-900 dark:text-gray-300 leading-relaxed">
                    {feature.description}
                  </p>
                </div>
        ...
        <div className="mt-16 text-center animate-on-scroll opacity-0" style={{ animationDelay: '600ms' }}>
          <div className="bg-white dark:bg-gray-800 inline-block px-8 py-6 rounded-2xl border border-raizing-teal-900/20 shadow-lg">
            <p className="text-raizing-teal-900 dark:text-white text-lg mb-4">
              Join <span className="text-raizing-maroon-600 dark:text-raizing-maroon-400 font-bold text-2xl">500+</span> companies already transforming their business with AI
            </p>
            ...
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6 mb-6">
              {[
                ...
                <div key={idx} className="flex items-center gap-2 px-4 py-2 bg-raizing-cream-50 dark:bg-gray-700 rounded-lg border border-raizing-teal-900/20 dark:border-gray-600 transition-colors duration-300">
                  <span className="text-raizing-maroon-600 dark:text-raizing-maroon-400 font-bold">{badge.icon}</span>
                  <span className="text-sm text-raizing-teal-900 dark:text-gray-200">{badge.name}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-4 sm:gap-6 md:gap-8 mt-6 mb-6">
              {['Microsoft', 'Google', 'Amazon', 'IBM'].map((company, idx) => (
                <div key={idx} className="text-raizing-teal-900 dark:text-gray-300 font-semibold text-sm sm:text-base opacity-70 hover:opacity-100 hover:text-raizing-maroon-600 dark:hover:text-raizing-maroon-400 transition-all cursor-pointer">
                  {company}
                </div>
              ))}
            </div>
