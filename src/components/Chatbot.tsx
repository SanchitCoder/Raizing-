      ...
      >
        <div className="bg-gradient-to-r from-raizing-maroon-500 to-raizing-maroon-600 p-4 sm:p-6 text-raizing-cream-200 flex-shrink-0">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="font-bold text-xs sm:text-sm sm:text-base sm:text-lg truncate">Raizing AI Assistant</h3>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-3 sm:space-y-4 bg-raizing-cream-50 dark:bg-gray-900">
          {messages.map((message) => (
            ...
            >
              <div
                className={`max-w-[80%] px-4 sm:px-6 py-2 sm:py-3 rounded-2xl ${
                  message.sender === 'user'
              ...
              >
                ...
                <p
                  className={`text-xs mt-1 ${
                    message.sender === 'user' ? 'text-raizing-cream-300' : 'text-raizing-teal-900'
                  }`}
                >
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="bg-white dark:bg-gray-800 border border-raizing-teal-900 px-4 sm:px-6 py-2 sm:py-3 rounded-2xl">
        ...
        <div className="p-3 sm:p-4 bg-white dark:bg-gray-800 border-t border-raizing-teal-900 flex-shrink-0">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 sm:px-4 sm:px-4 sm:px-6 py-1.5 sm:py-2 sm:py-3 text-xs sm:text-sm sm:text-sm sm:text-base rounded-lg border border-raizing-teal-900 focus:border-raizing-maroon-500 focus:ring-2 focus:ring-raizing-maroon-500/20 outline-none transition-all"
              disabled={isLoading}
            />
            <button
              onClick={sendMessage}
              disabled={!inputValue.trim() || isLoading}
              className="px-3 sm:px-4 sm:px-4 sm:px-6 py-1.5 sm:py-2 sm:py-3 bg-gradient-to-r from-raizing-maroon-500 to-raizing-maroon-600 text-raizing-cream-200 rounded-lg hover:from-raizing-maroon-600 hover:to-raizing-maroon-700 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
            >
